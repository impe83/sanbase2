defmodule Sanbase.Clickhouse.Metric do
  ...
  def table_data(_metric, %{datetime_selector: []}), do: {:ok, []}

  def table_data(metric, %{datetime_selector: datetime_selector}, opts) do
    aggregation = Keyword.get(opts, :aggregation, nil) || Map.get(@aggregation_map, metric)
    filters = Keyword.get(opts, :additional_filters, [])
  end
  ...
  @impl Sanbase.Metric.Behaviour
  def available_table_structured_metrics(), do: @table_structured_metrics_name_list
